...
export function mergeRatios(liveResp: unknown, dbResp: unknown): RatioSet | null {
  ...
  const dbPeriod: RatioPeriod | null = dbLabel ? { label: dbLabel, ttm: false } : null;
...
/** Gross dividends paid in the last 365 days and the resulting yield at `price`. */
export function trailingDividendYield(
  ...
  const perShare = paid.reduce((sum, d) => sum + (d.grossPerShare ?? 0), 0);
...
function normalizeSentiment(raw: unknown): Sentiment | null {
  const s = asStr(raw)?.toLocaleLowerCase("tr-TR");
  if (!s) return null;
  if (s.startsWith("poz") || s.startsWith("pos")) return "positive";
  if (s.startsWith("neg")) return "negative";
  if (s.startsWith("nöt") || s.startsWith("not") || s.startsWith("neu")) return "neutral";
  return null;
}

function normalizeImpact(raw: unknown): Impact | null {
  const s = asStr(raw)?.toLocaleLowerCase("tr-TR");
  if (!s) return null;
  if (s.startsWith("düş") || s.startsWith("dus") || s === "low") return "low";
  if (s.startsWith("ort") || s.startsWith("med")) return "medium";
  if (s.startsWith("yük") || s.startsWith("yuk") || s === "high") return "high";
  return null;
}

/** `GET /news/{t}?hours=N` → { news: [{ title, url, source, published_at, sentiment, impact, rationale }] }. */
