    ...
    <SectionCard
      ...
      actions={
        overall != null && !pending ? (
          <span className="inline-flex items-center gap-1.5 rounded-lg border border-amber-500/20 bg-amber-500/10 px-2 py-1 text-[11px] font-semibold text-amber-700 dark:text-amber-400">
            {t("score.overall")}
      ...
      {pending ? (
        ...
        <>
          ...
          <button
            ...
            className="mt-3 inline-flex items-center gap-1 rounded-md text-[11px] font-semibold text-primary hover:underline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring/60"
          >
            <ChevronDown aria-hidden className={cn("h-3.5 w-3.5 transition-transform", showMethod && "rotate-180")} />
            {t("score.howCalculated")}
          </button>
          {showMethod ? (
            <div id={methodId} className="mt-2 space-y-3 rounded-xl bg-muted/30 p-3 text-[11px] text-muted-foreground">
