...
export const NAV_ITEMS: readonly NavItem[] = [
  { key: "dashboard", labelKey: "nav.dashboard", icon: LayoutDashboard, href: () => "/", isActive: (p) => p === "/" },
  { key: "events", labelKey: "nav.events", icon: Newspaper, href: () => "/events", isActive: (p) => p.startsWith("/events") },
  {
    key: "stock",
  ...
  { key: "combined", labelKey: "nav.combinedAnalysis", icon: Columns2, href: (ticker) => `/analiz/${ticker}`, isActive: (p) => p.startsWith("/analiz") },
  { key: "macro", labelKey: "nav.macroEconomy", icon: Globe, href: () => "/makro", isActive: (p) => p.startsWith("/makro") },
  { key: "screening", labelKey: "nav.screening", icon: Search, href: () => "/tarama", isActive: (p) => p.startsWith("/tarama") },
];
