...
class NewsItem(Base):
    ...
    published_at: Mapped[datetime] = mapped_column(DateTime(timezone=True), nullable=False)
    sentiment: Mapped[str | None] = mapped_column(String(20), nullable=True)  # 'pozitif' | 'notr' | 'negatif'
    impact: Mapped[str | None] = mapped_column(String(20), nullable=True)  # 'yuksek' | 'orta' | 'dusuk'
    rationale: Mapped[str | None] = mapped_column(String(300), nullable=True)
    created_at: Mapped[datetime] = _created_at()
...
class AIUsage(Base):
    """One row per billed LLM provider call (news classification).

    The shared daily AI budget is ``SUM(cost_usd)`` since Europe/Istanbul midnight, so the
    API and worker processes see each other's spend (see ``src.adapters.llm.DailyBudget``).
    ``kind`` values 'report' and 'batch' only occur in rows written before the AI report
    feature was removed.
    """

    __tablename__ = "ai_usage"
    __table_args__ = (
        CheckConstraint("kind IN ('report', 'classification', 'batch')", name="ck_ai_usage_kind"),
    ...
    kind: Mapped[str] = mapped_column(String(20), nullable=False)  # 'report' | 'classification' | 'batch'
    ticker: Mapped[str | None] = mapped_column(String(20), nullable=True)
    input_tokens: Mapped[int] = mapped_column(Integer, nullable=False)
    output_tokens: Mapped[int] = mapped_column(Integer, nullable=False)  # incl. thinking tokens
