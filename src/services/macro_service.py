...
def _policy_rate_payload_from_rows(rows: Sequence[MacroObservation]) -> dict[str, Any]:
    latest = rows[-1]
    history = [
        {"date": f"{r.observation_date.isoformat()}T00:00:00", "borrowing": None, "lending": float(r.value)} for r in rows
    ]
    return {
        "source": "TCMB",
        "source_url": tcmb_adapter.RATE_PAGE_URLS["policy"],
...
async def get_policy_rate_store_first(session: AsyncSession) -> dict[str, Any]:
    ...
    now = utcnow()
    rows = await MacroRepository(session).history("tcmb.policy_rate", limit=24, ascending=True)
    max_age = timedelta(hours=settings.macro_max_age_rates_hours)
...
async def _store_inflation_history(session: AsyncSession) -> tuple[list[dict[str, Any]], datetime | None]:
    repo = MacroRepository(session)
    yoy_rows = await repo.history("tuik.cpi.yoy", ascending=False)
    if not yoy_rows:
        return [], None
    mom_rows = await repo.history("tuik.cpi.mom", ascending=False)
...
def _inflation_payload_from_history(history: list[dict[str, Any]]) -> dict[str, Any]:
    top = history[0]
    latest = {
        "date": top["Date"][:10],
...
async def _write_through_inflation(session: AsyncSession, payload: dict[str, Any]) -> None:
    rows: list[ObservationRow] = []
    for entry in payload.get("tufe_history") or []:
        day = _parse_iso_date(entry.get("Date"))
        if day is None:
            continue
        if entry.get("YearlyInflation") is not None:
            rows.append(ObservationRow("tuik.cpi.yoy", day, _decimal(entry["YearlyInflation"]), "tuik"))
        if entry.get("MonthlyInflation") is not None:
            rows.append(ObservationRow("tuik.cpi.mom", day, _decimal(entry["MonthlyInflation"]), "tuik"))
    if rows:
        await MacroRepository(session).upsert_observations(rows)
        await session.commit()
...
async def get_inflation_store_first(session: AsyncSession) -> dict[str, Any]:
    """Store-first replacement for ``src.adapters.macro.get_inflation()`` (TÜFE only —
    matches the current live function; ÜFE/PPI is store-only, see :func:`ingest_inflation`)."""
    now = utcnow()
    ...
    if history and oldest_fetched_at is not None and (now - oldest_fetched_at) <= max_age:
        payload = _inflation_payload_from_history(history)
        return with_meta(
            payload,
            DataMeta(
