...
AKSA için "Aksa Enerji") ve başka kuruluşlar ("Kıbrıs Türk Hava Yolları") elenir.
Spor kulübü olmayan şirketlerde spor bağlamlı başlıklar (Anadolu Efes basketbol
takımı, "Türkiye Sigorta Basketbol Süper Ligi") ve adı taşıyan başka kurumlar
("Türkiye Sigorta Birliği", "... Spor Kulübü") şirket haberi sayılmaz; adı bir spor
takımıyla aynı olan şirketlerde (AEFES) finans bağlamı aranır. Kod biçiminde geçen
başlıklar ("AEFES") her zaman eşleşir.
"""
...
_FINANCE_PREFIXES = (
    ...
    "tahvil", "endeks", "yatirimci", "lot ", "istirak",
)
...
# Other organisations whose names contain a listed company's name; blanked before
# matching like ``exclude_names`` ("Kıbrıs Türk Hava Yolları" is not THY).
_OTHER_ORGANISATIONS = ("Kıbrıs Türk Hava Yolları",)
# Listed sports clubs: match and transfer news is their business news.
SPORTS_CLUB_TICKERS = frozenset({"BJKAS", "FENER", "GSRAY", "TSPOR"})
# Companies whose name is also a sports team's name ("Anadolu Efes" basketball): like a
# generic one-word name, a headline counts only with finance context.
TEAM_NAME_TICKERS = frozenset({"AEFES"})
# The name followed by one of these is another body carrying it ("Türkiye Sigorta Birliği",
# "Türk Telekom Basketbol", "... Spor Kulübü"), not the listed company.
_ORGANISATION_WORDS = (
    "birligi", "vakfi", "dernegi", "spor", "kulubu", "ligi", "super ligi", "basketbol", "voleybol", "futbol",
    "stadyumu", "arena", "muzesi", "kupasi", "open", "takimi", "basketball", "cup",
)
_ORGANISATION_AFTER_RE = re.compile(
    r"(?:['?][a-z]*)?\s+(?:" + "|".join(re.escape(w) for w in _ORGANISATION_WORDS) + r")(?![a-z0-9])"
)
# Sports context (folded). For any other company such a headline is about a team,
# league or sponsorship carrying its name (Anadolu Efes basketball, "Türkiye Sigorta
# Basketbol Süper Ligi", Türk Telekom Ankara), not about the listed company.
_SPORTS_PREFIXES = (
    "basketbol", "futbol", "voleybol", "euroleague", "eurocup", "super lig", "super cup", "sampiyonlar ligi",
    "avrupa ligi", "avrupa kupa", "milli takim", "play-off", "playoff", "final four", "final-four", "derbi",
    "galibiyet", "maglubiyet", "deplasman", "fikstur", "skorer", "teknik direktor", "antrenor", "basantrenor",
    "turnuva", "kupa", "basketball", "football", "volleyball",
)
_SPORTS_WORDS = ("mac", "maci", "macina", "macinda", "maclari", "maclar", "lig", "ligi", "ligde", "ligin", "mvp", "cup")
_SPORTS_RE = re.compile(
    r"(?<![a-z0-9])(?:"
    + "|".join(re.escape(p) for p in _SPORTS_PREFIXES)
    + r"|(?:"
    + "|".join(_SPORTS_WORDS)
    + r")(?![a-z0-9]))"
)
...
def is_relevant(
    ...
    text = f"{title} \n {snippet}"
    folded = fold(text)
    for other in (*exclude_names, *_OTHER_ORGANISATIONS):
    ...
    if not name:
        return False
    club = ticker.upper() in SPORTS_CLUB_TICKERS
    mentions = [
        m
        for m in re.finditer(rf"(?<![a-z0-9]){re.escape(name)}(?![a-z0-9])", folded)
        if club or not _ORGANISATION_AFTER_RE.match(folded, m.end())
    ]
    if not mentions:
        return False
    if not club and _SPORTS_RE.search(folded):
        return False
    if len(name.split()) >= 2 and ticker.upper() not in TEAM_NAME_TICKERS:
        return True
