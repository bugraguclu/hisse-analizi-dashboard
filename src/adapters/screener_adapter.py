...
def screen_all_stocks_sync() -> Any:
    """Every listed BIST stock (İş Yatırım), including those priced under 1 TL.

    Without any criterion borsapy adds "price > 1 TL", which silently drops
    listed penny stocks (GSRAY, TSPOR, IHLAS, ...); an explicit full price
    range keeps them.
    """
    from borsapy.screener import Screener

    return Screener().add_filter("price", min=0, max=1_000_000).run()
...
def _merge_live_market_fields(records: list[dict], market_by_symbol: dict[str, dict]) -> list[dict]:
    ...
    ``criteria_7`` is İş Yatırım's last *closing* price; ``close`` /
    ``change_pct`` are the live TradingView values for today.
    """
    enriched: list[dict] = []
