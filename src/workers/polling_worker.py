...
from src.adapters.kap import KAPAdapter, KapUnavailableError
...
# Sources whose adapters hit the network once per cycle, not once per company.
_SHARED_FETCH_SOURCES = frozenset({"kap"})
...
async def _poll_company(
    ...
    try:
        ...
        # A shared KAP download failing fails every company alike: logged once per cycle.
        log = logger.debug if isinstance(e, KapUnavailableError) else logger.warning
        log("poll_company_error", source=source.code, ticker=ticker, error=str(e))
        stats["ok"] = False
...
async def run_source_cycle(
    ...
    *,
    ...
    async with source_lock(source_code) as acquired:
        ...
        async def poll_one(company: Company) -> dict[str, Any]:
            ...
            async with semaphore:
                ...
                if source.code not in _SHARED_FETCH_SOURCES:
    ...
    ended_at = utcnow()
    first_error = next((r["error"] for r in summary["results"] if r.get("ok") is False), None)
    logger.info(
        "poll_cycle_complete",
        source=source_code,
