...
async def test_unfiltered_screen_and_company_list_keep_stocks_priced_under_one_lira(monkeypatch):
    ...
    class FakeScreener:
        ...
        def run(self):
            return pd.DataFrame([{"symbol": "GSRAY", "name": "Galatasaray", "criteria_7": 0.91},
                                 {"symbol": "THYAO", "name": "THY", "criteria_7": 293.5}])

    async def no_market(symbols, columns, **kwargs):
        return {}

    monkeypatch.setattr(borsapy.screener, "Screener", FakeScreener)
    monkeypatch.setattr(screener_adapter, "tradingview_scan", no_market)
    ...
    assert added == [("price", 0, 1_000_000)] * 2
    assert [row["symbol"] for row in screen["results"]] == ["GSRAY", "THYAO"]
    assert screen["results"][0]["close"] == 0.91
