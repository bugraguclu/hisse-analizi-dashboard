...
`notifications`, `sources`, `polling_state`) değişmedi.
...
`src/adapters/news.py`, `src/adapters/screener_adapter.py`,
...
> **Durum (26.09.2026): birleştirildi.** Aşağıdaki kurallar uygulandı (taban `2080f8b`, 020 → `down_revision = "011"`).
> Notlardan sapmalar, master'ın 25.09'da eklediği davranışları korumak için:
> - `/macro/policy-rate` ve `/macro/inflation` depo yanıtları master'ın canlı yanıtındaki alanları da üretir:
>   `changes`/`last_change` (tüm faiz değişiklikleri; write-through artık `changes`'i de saklar) ve
>   `ufe_latest`/`ufe_history` (depodaki `tuik.ppi.*` serilerinden) — `macro_service`.
> - `index_adapter.py`: gövdeler branch'ten (depo öncelikli), master'ın isteğe bağlı parça bekleme sınırı
>   (`OPTIONAL_PART_GRACE`, grafik barları geldikten sonra 1 sn) korunur; `price.py` master'ın ortak TradingView
>   grafik soketini (`tradingview_chart.get_history`) kullanmaya devam eder.
> - Master'ın `tests/unit/test_chart_warmup.py` testleri depo öncelikli yola taşındı; büyüme koruması testi
>   `analysis_service.compute_financial_ratios`'u çağırır.
