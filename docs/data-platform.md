...
| İş Yatırım günlük geçmiş | `GET .../Common/Data.aspx/HisseTekil?hisse=GARAN&startdate=15-09-2026&enddate=22-09-2026` | 0,1 sn; resmî günlük seri: `HGDG_KAPANIS` (kapanış), `HGDG_AOF` (AOF/VWAP), `HGDG_MIN/MAX`, `HGDG_HACIM` (TL ciro), `PD` (piyasa değeri), `HAO_PD` (halka açık PD), `DOLAR_BAZLI_FIYAT`, `END_DEGER` (aynı gün XU100). **Doğrulandı (WS2):** `HG_*` = işlem gördüğü fiyat (düzeltmesiz; TradingView ile 566/566 birebir, bedelsiz öncesinde HG × bölünme katsayısı), `HGDG_*` = bölünme **ve temettü** düzeltmeli (temettü sonrası TV'den %0,6–2,5 sapar). Bu yüzden İşY yalnızca `turnover` (`HG_HACIM`, TL) ve `vwap` (`HG_AOF`, saklanan baza ölçeklenir), kapanış çapraz kontrolü ve boşluk yedeği için kullanılır; çok yıllık pencerelerde 1–6 sn yanıt verir ve günün satırı akşam geç (D+1) gelir. Hacim birimleri: TV `volume` = lot (bölünme düzeltmeli; Yahoo ile birebir), İşY `HG_HACIM` = TL, lot = `HG_HACIM / HG_AOF` (TV/İşY lot oranı medyan 0,99999994); TV `Value.Traded` = kapanış × lot, gerçek ciro değildir. |
...
| `financial_statements` | (company_id, source, period, statement_type) | WS3 | `source ∈ {kap, isyatirim}`, `period = "YYYY/MM"`, `statement_type ∈ {balance_sheet, income_stmt, cash_flow}`, `items_json` = sıralı satırlar `{code, label, key, value}` (TL). `restated` + `restatement_factor` (= KAP / İşY) İşY IAS 29 kolonları için; kalemler yalnızca KAP ile son iki ortak dönemde eşleşen İşY satırlarından beslenir (banka/sigorta kapsam farkında yalnızca KAP). `companies.statement_template ∈ {industrial, bank, insurance, ...}`. Liste uçları `meta`'yı `X-Data-Meta` başlığında taşır. |
...
| | | `market.bars.reconcile` | `MARKET_RECONCILE_AFTER`'dan (vars. 20:00) sonra günde bir (+ başlangıçta telafi): İşY `HG_KAPANIS` ile kapanış mutabakatı, `turnover`/`vwap` doldurma, İşY yedeğiyle boşluk kapatma. |
